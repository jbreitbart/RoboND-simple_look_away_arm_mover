...
void move_arm_center() {
    ...
    if (!client.call(srv)) ROS_ERROR("Failed to call service safe_move");
}
...
void look_away_callback(const sensor_msgs::Image img) {
    ...
    if (uniform_image == true && moving_state == false) move_arm_center();
}
...
int main(int argc, char **argv) {
    ...
    // Subscribe to /simple_arm/joint_states topic to read the arm joints position inside the joint_states_callback
    // function
